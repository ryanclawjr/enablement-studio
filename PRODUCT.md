...
GET `/` is Role Source — the table, not a landing or product picker. Craft is dark Tablework: canvas about `#0d0d0d` / `#111` with a faint square grid, one yellow accent (`#fffd63` / `#F5C518`) on Run and the current path step. Chrome is a tight top bar: wordmark Tablework, one status pill (`this machine · 127.0.0.1 · offline`), and a receding `Call Next · Critic Next` text line. Path chrome (Source · Graph · Objectives · Outline · Practice · Quiz) sits under the bar. The page is objects. Source is the first object: a document-sized dark textarea, label "Job, SOP, or policy", project, pill Run, ghost LLM, then Run Harborline · EXAMPLE DATA as a text action. After Run, Graph / Objectives / Outline / Practice / Quiz are crafted cards (rounded, hairline, CSS hover lift) revealed one step at a time. Hover is CSS only (200–300ms). Board about 42rem, left-aligned. Wordmark and chrome say Tablework, not Enablement Studio. Do not ship beige/stone/steel. Do not ship a hero, spotlight, pricing, or three equal-weight product cards.
