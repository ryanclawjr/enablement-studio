...
GET `/` is Role Source — the table, not a landing or product picker. Craft tokens are measured from microinteractions.co: chrome `#16161D`, table panel `#0B0B0E` with a faint 1px square grid on the table only, cards `#1C1C22` with hairline and large radius, accent `#FFE52F` with black text on Run and the current path step. Chrome is a tight top bar: wordmark Tablework, one status pill (`this machine · 127.0.0.1 · offline`), and a receding `Call Next · Critic Next` text line. Path chrome (Source · Graph · Objectives · Outline · Practice · Quiz) sits under the bar. The page is objects. Source is the first object on the table: a document-sized dark textarea, label "Job, SOP, or policy", project, pill Run, ghost LLM, then Run Harborline · EXAMPLE DATA as a text action. After Run, each later step is one rounded dark card like that collection row, revealed one at a time. Hover is CSS only (200–300ms). Board about 42rem, left-aligned. Wordmark and chrome say Tablework, not Enablement Studio. Do not ship beige/stone/steel. Do not ship a hero, spotlight, pricing, or three equal-weight product cards.
