...
GET `/` is Role Source. Home is one job document on a full-bleed dark table. The canvas is `#0E0E12` with a 1px square grid on the whole page (about 64px, faint white). The sheet is centered, about 820px, `#16161D`, hairline `#2A2A33`. Quiet SOURCE kicker, a 16px textarea, yellow Run, ghost LLM, Harborline as muted text. Versions are small dots in the sheet footer, current dot `#FFE52F`. The Role path is six object cards sitting on that grid: Source, Graph, Objectives, Outline, Practice, Quiz. The current card has a yellow hairline and yellow dot. Clicking a card uses the same `?run=` / `?step=` query as today's path. No second row of pills under the wordmark. No two-column studio and versions rail. Chrome stays tight: Tablework, one status pill (`this machine · 127.0.0.1 · offline`), and `Call Next · Critic Next`. Accent `#FFE52F` with black text is rare: Run and the current object. Fonts are local Outfit and Syne, not fonts.googleapis.com. Hover is CSS only (240ms ease, translateY(-2px)). Wordmark says Tablework, not Enablement Studio. Do not ship beige/stone/steel. Do not ship a hero, spotlight, pricing, or three equal-weight product cards.
